    ...
    <ul className="radar-bars">
      {rows.map((row) => (
        <li key={row.tag}>
          ...
          <span className="radar-bar" aria-hidden><span style={{ width: `${Math.round((row.recent / max) * 100)}%` }} /></span>
...
export default async function RadarPage({ params }: { params: Promise<{ lang: Locale }> }) {
  ...
  return (
    <PageShell kicker={t.kicker} title={t.title} intro={t.intro}>
      <div className="split-2" style={{ marginTop: 32 }}>
      ...
      {radar.watchlist.length ? <section style={{ marginTop: "var(--section-gap)" }}><Wire items={radar.watchlist} locale={locale} /></section> : null}
      <section style={{ marginTop: "var(--section-gap)" }}>
        <h2>{t.strongest}</h2>
        <ul style={{ listStyle: "none", padding: 0 }}>
          {radar.strongest.map((article) => <IssueRow key={article.slug} href={localePath(locale, `/articles/${article.slug}`)} date={article.date} title={article.title} variant="meta" trailing={<span className="label">{article.signal_strength}</span>} />)}
        </ul>
      </section>
      {radar.cooled.length ? <section style={{ marginTop: "var(--section-gap)" }}><h2>{t.cooled}</h2><TopicBars rows={radar.cooled} locale={locale} /></section> : null}
      {pulse ? <section style={{ marginTop: "var(--section-gap)" }}><h2>{t.pulse}</h2><AIPulse pulse={pulse} locale={locale} /></section> : null}
      <section style={{ marginTop: "var(--section-gap)" }}>
        <h2>{t.timeline}</h2>
        <ul style={{ listStyle: "none", padding: 0 }}>{radar.timeline.map((article) => <IssueRow key={article.slug} href={localePath(locale, `/articles/${article.slug}`)} date={article.date} title={article.title} variant="meta" />)}</ul>
      </section>
    </PageShell>
  );
}
