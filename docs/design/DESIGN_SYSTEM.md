...
Authentic UI is always rendered from production code and data. Article images keep intrinsic geometry and local/static delivery. Generated-media production is queued after mandatory current provider research; missing media renders as no media, never as a fake placeholder. Selected assets must be optional, locally committed, intrinsically sized, responsive, and accompanied by an explicit accessible-text classification.
