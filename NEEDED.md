...
- [ ] **Apply to Seznam Newsfeed** — register a Seznam Partner account on the operator's IČO at partner.seznam.cz and submit the site with `/feed.xml`; the reader already has HTTPS, a favicon, responsive pages and the Atom feed. Review takes up to 15 business days and a rejected site waits 30 days, so apply before the newsletter launch rather than after. [imp:4] [owner:me] [time:30m] [kind:setup]
- [ ] **Provide an editorial contact address** — Seznam Newsfeed and Google News both expect a visible editorial contact, and the reader has none: `lib/brand.ts` carries no address and About and the footer show none. Decide the address, then it goes into `brand`, About and the footer in one change. The repository cannot invent one. [imp:3] [owner:me] [time:10m] [kind:decision]
- [ ] **Register the two sitemaps in Search Console** — `/news-sitemap.xml` now lists the two publishing days ending on the newest edition and `/llms.txt` indexes the publication for agents; `robots.txt` already names both sitemaps, and Search Console accepts the news sitemap only once it is submitted there. [imp:2] [owner:me] [time:10m] [kind:setup]
...
- The distribution pack of 2026-09-16: a Google News sitemap anchored on the newest
  edition's date, the root robots block that allows large image previews for
  Discover, `/llms.txt` as the agent-readable index, and a delivery warning when
  a photograph or illustration is narrower than the 1200 px Discover floor. The
  package still ships; the run log says why Discover will not show it.
