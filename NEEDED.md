...
- [ ] **Decide whether the practical block goes live** — the reader half shipped on 2026-09-17 and currently renders nothing: none of the 35 committed editions carries a `practical:` key. Quorum's `config/edition-quality.json` sets `article.practicalItem: false`, and `orchestrator/src/edition/write.ts` feeds that flag to `writeToolInputSchema`, which drops the field from the tool contract entirely when it is false — so the desk is never asked for a practical item and cannot file one. Turning it on changes what the writing model is asked to produce on every run, which draws on quorum's $25 model share and its $1.00 daily pace, so this is a spend decision rather than a config edit. The comment above that flag still says DNESKAi does not render the field, which stopped being true this session. The four items below only matter once this one is on. [imp:3] [owner:me] [time:15m] [kind:decision]
- [ ] **Confirm or replace the practical block's Czech copy** — the kicker reads "k vyzkoušení" for a `daily` block and "nástroje týdne" for `friday-tools`; the headings are "Co si dnes můžete zkusit." and "Tři nástroje a jeden prompt."; the three kind labels are "Prompt", "Nástroj" and "Jak na to". That follows the house pattern of a lowercase mono kicker above a full sentence ending in a period, but it was written here rather than chosen by you. All ten strings sit in `lib/i18n/dictionaries.ts` in both locales, so replacing any of them is a dictionary edit with no layout consequence. [imp:2] [owner:me] [time:15m] [kind:decision]
- [ ] **Decide which non-HTML surfaces carry the practical block** — it renders on Today and on the article page only. The per-edition markdown in `lib/distribution/llms.ts` and the email edition in `lib/distribution/newsletter.ts` both already carry Briefs and Watchlist and both omit this block, and the markdown builder's own contract says it shows the same editorial content the reading page shows, in the same order, so that is the clearest gap. Print is a wider question: `components/PrintArticle.tsx` renders the body and the source list and nothing else, so it omits Briefs and Watchlist too, and adding one section there sets a precedent for the other two. `/api/today.json` publishes `schema_version: 2`, so a new field there is a distribution-contract change and deserves a deliberate version decision rather than a silent addition. The email channel stays inert until `config/subscribe.json` is filled either way. [imp:2] [owner:me] [time:20m] [kind:decision]
- [ ] **Confirm the practical block's placement in the main column** — the request was for the right rail; it went in as a peer section in the main column instead. `--rail-width` is 300 px, the contract allows a 600-character body and four items in the `friday-tools` variant, and the two widgets already in the rail are deterministic one-liners. A 300 px column is not sized for four 600-character bodies. If you want it in the rail anyway, that is a placement change rather than a rewrite. [imp:2] [owner:me] [time:10m] [kind:decision]
- [ ] **Decide whether `pnpm check:content` should also enforce upstream's authoring arithmetic** — `practicalErrors` checks what a reader depends on: the variant, one to four items, the kinds, duplicate titles, a URL written into a title or body, and a `source_url` the same file does not cite in `sources` or `wire`. It deliberately leaves out the 40-600 character body bound and the one-item / three-tools-plus-a-prompt composition, because both are written and enforced upstream and a second copy here could only drift and start rejecting editions that are correct. If you would rather have both sides check them anyway, it is a bounded addition to that function. [imp:1] [owner:me] [time:10m] [kind:decision]
...
- The practical block renders. `resolvePractical` normalises the optional
  frontmatter field, `PracticalBlock` shows it as a peer section on Today and
  on the article page, and `pnpm check:content` fails on a block a reader could
  not trust. It adds no client JavaScript. It ships dark, because no edition
  carries the field yet; the five practical-block items above are what is open.
