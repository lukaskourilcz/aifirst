...
The Caught Up product, operations work and design overhaul are merged into local
`main`. The public site builds with no private credentials, but publishing a new
edition requires the operator setup below. Each task carries an importance
score `[imp:N]` (1–5, 5 = highest) and an owner marker.
...
- [ ] **Research and select a safe free or low-cost media generator** — compare at least three current free-tier, open-source/local, or inexpensive options using official pricing, license and privacy sources. Record commercial rights, watermark and public-gallery behavior, limits, original-download access and cost per usable image. Follow `docs/design/GENERATIVE_MEDIA_ART_DIRECTION.md` and `docs/design/GENERATED_MEDIA_ASSET_MANIFEST.md`. Any purchase requires operator approval. `[imp:2]` `[owner:ai]` `[time:2h]` `[kind:decision]`
